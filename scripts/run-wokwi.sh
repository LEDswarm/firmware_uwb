...
if [ "${WOKWI_PROJECT_ID}" == "" ]; then
    wokwi-server --chip esp32 target/${ESP_ARCH}/${BUILD_MODE}/firmware-rust-esp32-uwb
else
    wokwi-server --chip esp32 --id ${WOKWI_PROJECT_ID} target/${ESP_ARCH}/${BUILD_MODE}/firmware-rust-esp32-uwb
fi